...
class AddReferenceDetailsScreen extends StatefulWidget {
  final Notes note;
  final bool isFromUpdate;
  final ReferenceModel? referenceModel;
  final ReferenceModel? referenceModel2;
  final String referenceID;

  const AddReferenceDetailsScreen(this.note, this.referenceID,
      {this.referenceModel, this.referenceModel2, this.isFromUpdate = false, Key? key})
      : super(key: key);

  @override
  State<AddReferenceDetailsScreen> createState() => _AddReferenceDetailsScreenState();
}

class _AddReferenceDetailsScreenState extends State<AddReferenceDetailsScreen> {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Consumer<NoteProvider>(
        builder: (context, noteProvider, child) => Container(
          ...
          child: CustomButton(
              btnTxt: widget.isFromUpdate ? 'Update' : 'Add',
              onTap: () {
                if (textEditingController.text.isEmpty) {
                  ...
                  if (widget.isFromUpdate) {
                    ...
                    noteProvider
                        .updateNoteDetails(
                            referenceModel, widget.note.noteId, widget.referenceID, context, widget.referenceModel2!.referenceId)
                        .then((value) {
                      if (value) {
                    ...
                    referenceModel = ReferenceModel(time, textEditingController.text);

                    noteProvider
                        .addNoteDescription(
                            referenceModel, widget.note.noteId,widget.referenceID, widget.referenceModel!.referenceId,  context)
                        .then((value) {
                      if (value) {
                        textEditingController.clear();
                        Navigator.of(context).pop();
      ...
      appBar: AppBar(
        ...
        title:
            Text('${widget.isFromUpdate ? 'Update' : 'Add'} Reference Details', style: const TextStyle(color: Colors.black, fontSize: 18)),
      ),
      body: ListView(
        ...
        children: [
          CustomTextField2(
            ...
            hintText: 'Write Your reference details...',
            maxLines: null,
